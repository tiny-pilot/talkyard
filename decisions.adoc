...
computers.  Migth in the future get shipped off-site somehow?

The **audit log** includse what *humans* did, and is nice to read, for
auditors.  But it might include sensitive info, e.g. personal details — and so
should not be used for e.g. sending webhooks events. Might in the future get
shipped off-site somehow?
...
debugging. — It's part of the Ty application, and updates to the event log,
might resul in new entries in the debug log.
...
notification. Whilst events are per Ty site.
